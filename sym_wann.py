...
from _utility import num_cart_dim
...
class SymWann:
    ...
    def symmetrize(self):
        ...
        logfile.write('~~~~~Additional R vectors~~~~~~\n')
...
class WannAtomInfo():
    ...
    def __str__(self):
        return ";\n".join(f"{key}:  {value}" for key, value in self.__dict__.items() if key not in ["orb_position_on_atom_dic","ur","ul"])
