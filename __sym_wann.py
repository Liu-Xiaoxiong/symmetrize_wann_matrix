...
class sym_wann():
	...
	def full_p_mat(self,match_index,rot):
		...
		p_mat_small = p_mat[a:b,a:b]
		return p_mat_small,a,b#,wann_index_list[0],wann_index_list[-1]+1 
			...
			for rot in range(1,self.nsymm):
				...
				for atomran in range(self.num_wann_atom):
					...
					if new_Rvec in self.iRvec:
						...
						p_mat_b,bo,be = self.full_p_mat(match_index,rot)
						
						HH_new_tmp = np.round(np.dot(np.dot(np.conj(np.transpose(p_mat_a)),HH_R_copy[bo:be,bo:be,new_Rvec_index]),p_mat_b),decimals=6)
						print('??????')
						print(p_mat_b)
						print(HH_new_tmp)
						print(HH_R_copy[ao:ae,ao:ae,ir])
						print('??????')
						HH_R[ao:ae,ao:ae,ir] += HH_new_tmp
						flag[ao:ae,ao:ae,ir] += 1	
		self.nRvec=nRvec
		select=flag > 0
		select= flag == 0
		flag[select] = 1
